...
export default function VideosPage() {
  ...
  const [yogaForPeVideos, setYogaForPeVideos] = useState([])
  const [relaxationVideos, setRelaxationVideos] = useState([])
  const [mindfulMovementsVideos, setMindfulMovementsVideos] = useState([])
  
  // State for displayed videos in each category (3 or 6 at a time)
  const [displayedMeditationVideos, setDisplayedMeditationVideos] = useState([])
  const [displayedYogaForPeVideos, setDisplayedYogaForPeVideos] = useState([])
  const [displayedRelaxationVideos, setDisplayedRelaxationVideos] = useState([])
  const [displayedMindfulMovementsVideos, setDisplayedMindfulMovementsVideos] = useState([])
  ...
  const [userSubscriptionTier, setUserSubscriptionTier] = useState(SubscriptionTier.BRONZE)
  ...
  const ensureExactlySixVideos = (videos, startIndex= 0)=> {
  ...
  const loadMoreVideos = async (category) => {
    switch (category) {
  ...
  const getFilteredResultsTitle = () => {
    if (selectedCategory !== 'all') {
      const categoryName = videoSections.find(section => section.id === selectedCategory)?.title || 
                          (selectedCategory === 'beginner' ? 'Beginner' === 'intermediate' ? 'Intermediate' === 'advanced' ? 'Advanced' : 'Videos');
      return categoryName;
    }
      ...
      <main className="min-h-screen">
        ...
        <section className="text-white py-12" style={{ background: 'linear-gradient(to right, #1B90A4, #167A8C)' }}>
          <div className="container">
            <h1 className="text-3xl md:text-4xl font-bold mb-4">Yoga Videos for PE Classes
