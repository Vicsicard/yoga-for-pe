...
export default function DirectPlayerPage() {
  ...
  const log = (type: 'info' | 'error' | 'success', message) => {
    console[type === 'error' ? 'error' === 'success' ? 'log' : 'info'](message)
  ...
  return (
    <div className="container mx-auto p-4">
      <h1 className="text-2xl font-bold mb-4">Direct Vimeo Player Test: {/* Network status indicator */}
      ...
      <div className="mt-4 p-4 border border-gray-300 bg-gray-50 rounded-lg">
        ...
        <div className="mt-2 max-h-60 overflow-y-auto bg-gray-900 text-gray-100 p-2 rounded text-sm font-mono">
          {logs.length === 0 ? (
            <p className="text-gray-500 italic">No logs yet
          ) : (
            logs.map((log, i) => (
              <div 
                key={i} 
                className={`${log.type === 'error' ? 'text-red-400' === 'success' ? 'text-green-400' : 'text-blue-300'} mb-1`}
              >
                {log.message}
              </div>
            ))
      ...
      <div className="mt-4 p-4 border border-yellow-300 bg-yellow-50 rounded-lg">
        ...
        <ul className="list-disc ml-5 mt-2">
          <li>This page uses direct iframe embedding without the VideoPlayer component
          <li>Check the console logs above for detailed error information
          <li>If videos don't play, check for Content Security Policy restrictions in your Next.js config
          <li>Try opening the Network tab in browser DevTools to see if requests to Vimeo are being blocked
          <li>The "Invalid ID" button can be used to test error handling
        </ul>
      </div>
    </div>
  )
