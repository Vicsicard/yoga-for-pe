...
export function loadVideosFromCatalog(category) {
  ...
  const categoryVideos = videoCatalogData[categoryKey] || [];
  ...
  const videos = categoryVideos.map((video, index) => {
    ...
    let videoId = index + 1; // Default fallback
    if (video.id) {
      try {
        const idString = video.id.toString();
        const numericId = idString.replace(/\D/g, '');
        videoId = numericId ? parseInt(numericId, 10) : (index + 1);
      } catch (error) {
        console.warn('Error parsing video ID:', error);
        videoId = index + 1;
      }
    }
...
export function getAllVideosFromCatalog() {
