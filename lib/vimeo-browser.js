...
export async function getVideosByCategory(
  ...
  try: {
    ...
    const response = await makeVimeoRequest('videos', {
      query, // Search for videos with this tag
      page: String(page),
    ...
    // Transform Vimeo API response to our Video  catch (error) {
...
function getMetadataField(video, field): string: {
  ...
  const customField = video.metadata.connections.custom_fields.options.find(
    (option) => option.name.toLowerCase() === field.toLowerCase()
  );
...
function formatDuration(seconds)= Math.floor(seconds / 60);
  const remainingSeconds = seconds % 60;
  return `${minutes}:${remainingSeconds.toString().padStart(2, '0')}`;
}

// Helper function to determine subscription tier based on video tags or metadata
function getVideoTier(video): SubscriptionTier: {
  ...
  if (tags.some((tag) => tag.name.toLowerCase() === 'gold')) {
    return SubscriptionTier.GOLD;
  } else if (tags.some((tag) => tag.name.toLowerCase() === 'silver')) {
...
export async function hasAccessToVideo(video, userId?, userTier?): Promise: {
  try: {
...
export async function getFeaturedFreeVideos(): Promise: {
...
function getThumbnailFolder(category): string: {
  switch(category) {
...
export function ensureThumbnailPath(video): Video: {
...
export function determineVideoCategory(video): VideoCategory: {
