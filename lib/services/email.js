...
class EmailService {
  constructor() {
    ...
    // Also try port 2525 as a fallback for restrictive environments like Vercel
    const port = parseInt(process.env.SMTP_PORT || '587');
    
    this.transporters[EMAIL_PROVIDER.BLUEHOST] = nodemailer.createTransport({
      host: process.env.SMTP_HOST || 'mail.yogaforpe.com',
      port: port, // Try port 587 or 2525 instead of 465
      secure: port === 465, // Only true for port 465
      auth: {
        user: process.env.SMTP_USER || 'info@yogaforpe.com',
        pass: process.env.SMTP_PASSWORD,
      },
      // Add connection timeout settings - increased for serverless cold starts
